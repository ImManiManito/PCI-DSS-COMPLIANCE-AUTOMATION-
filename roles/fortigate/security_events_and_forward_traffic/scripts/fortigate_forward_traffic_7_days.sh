#!/usr/bin/env bash
# Recolecta los logs de tráfico (type=traffic subtype=forward) de los últimos 7 días.
# Se filtra día por día y se pagina con "execute log filter start" porque el total
# de registros (ej. 1604 encontrados) supera el límite de "view-lines" (máx. 1000).
...

{
    ...
    for day_offset in 0 1 2 3 4 5 6; do
        ...
        for page_start in "${PAGE_STARTS[@]}"; do
            ...
            echo "execute log filter category 0"
            echo "execute log filter field type traffic"
            echo "execute log filter field subtype forward"
...
bash "$SSH_WRAPPER" "$HOST" "$PORT" "$USER" "$PRIVATE_KEY" < "$COMMANDS_FILE" \
    | python3 "${SCRIPT_DIR}/parse_forward_traffic.py" "$DEVICE_NAME"
