#!/usr/bin/env bash
# Consulta TODOS los logs (sin filtrar blade ni action) en Check Point Infinity
# Events (Infinity Portal API) para un rango de fechas (últimos 7 días) y vuelca
# cada página de resultados JSON (una por línea) a stdout.
#
...
# 2) Consulta paginada de TODOS los logs (sin filtro de blade/action) del rango
#    de fechas indicado (últimos 7 días), trayendo todos los resultados.
PAGE_TOKEN=""
